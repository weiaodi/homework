          ...
          this.prisma.$queryRaw`
            ...
            async (result: Array<{ userId: number; username: string }>) => {
              ...
              const promise = idList.map(async (item, index) => {
                ...
                // todo - 在这里进行数据聚合
                result.forEach((item) => {
                  // 关键修复：将 BigInt 类型的 currentyear 转换为 Number 类型
                  const currentYearNum = Number(item.currentyear);
                  // 计算年份在source数组中的索引（现在都是 Number 类型，可正常运算）
                  const yearIndex = currentYearNum - (currentYear - year + 1);
